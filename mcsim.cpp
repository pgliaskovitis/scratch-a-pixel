...
// This is just a rotation on random phi around mu, keeping the theta 
// sampled by H-G constant
// [/comment]
...
void MCSimulation(double *&records, const uint32_t &size)
{
    ...
    for (int n = 0; n < nphotons; ++n) {
        ...
        while (w != 0) {
            ...
            z += s * muz;
            // [comment]
            // The photon packet loses energy (absorption)
            // [/comment]
...
int main(int argc, char **argv)
{
    ...
    // save image to file
    std::ofstream ofs; 
    ofs.open("./mcsim.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n" << size << " " << size << "\n255\n";
